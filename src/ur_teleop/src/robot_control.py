...
class MoveItController:
    def __init__(self):
        ...
        self.cmd_sub = rospy.Subscriber('/servo_server/delta_twist_cmds', TwistStamped, self.cmd_callback, queue_size=1)
    ...
    def execute_motion(self):
        ...
        while not rospy.is_shutdown():
            with self.cmd_lock:
                ...
                self.current_cmd = TwistStamped()  # Reset the command

            if cmd.twist.linear.x == 0 and cmd.twist.linear.y == 0 and cmd.twist.linear.z == 0 and \
               cmd.twist.angular.x == 0 and cmd.twist.angular.y == 0 and cmd.twist.angular.z == 0:
                self.executing = False
                return
            ...
            # Update position
            current_pose.position.x += cmd.twist.linear.x * self.linear_scale
            current_pose.position.y += cmd.twist.linear.y * self.linear_scale
            current_pose.position.z += cmd.twist.linear.z * self.linear_scale
            
            # Update orientation
            orientation_q = current_pose.orientation
            orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]
            (roll, pitch, yaw) = euler_from_quaternion(orientation_list)
            roll += cmd.twist.angular.x * self.angular_scale
            pitch += cmd.twist.angular.y * self.angular_scale
            yaw += cmd.twist.angular.z * self.angular_scale
